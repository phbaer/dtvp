...
_MAX_ADVISORY_DETAILS_CHARS = 6_000
...
# Ecosystems we recognise from OSV-style "Ecosystem:name" package entries.
_KNOWN_ECOSYSTEMS = frozenset(web_fetcher._ECOSYSTEM_HINTS.values())


def _truncate_advisory_text(text: str, limit: int = _MAX_ADVISORY_DETAILS_CHARS) -> str:
    """Keep advisory detail blocks within a predictable prompt budget."""
...
def _append_advisory_detail_block(
    ...
    """Record a unique advisory detail block for LLM input."""
...
def _collect_advisory_detail_blocks(
    ...
    """Collect raw advisory bodies that may contain useful markdown context."""
...
def _summarize_advisory_sources(advisories: dict) -> str:
    """Return a compact summary of which advisory sources were used."""
    raw = advisories.get("raw") or {}
...
def _collect_missing_advisory_inputs(advisories: dict) -> list[str]:
    """Identify advisory fields that are important for downstream analysis."""
    missing: list[str] = []
...
def _build_advisory_analysis_input(
    vuln_id: str,
    advisories: dict,
    user_guidance: str = "",
) -> str:
    """Format advisory context for LLM analysis while preserving markdown bodies."""
...
def _list_project_manifests(repo_path: str | None) -> list[str]:
    """Return discovered manifest files for the current project path.

    Includes top-level and nested manifests (up to a shallow depth) so
    monorepos still provide meaningful ecosystem hints.
    """
    ...
    for root, dirs, files in os.walk(repo_path):
        ...
        for fname in files:
            if fname in web_fetcher._ECOSYSTEM_HINTS:
                rel_path = (
                    fname if rel_root == "." else os.path.join(rel_root, fname)
                )
