...
2. Validates service authentication before exposing the API.
3. Loads and hot-reloads `config/repos.yaml` for component-to-repository mappings.
4. Validates the prompt bundles used by the LLM-assisted steps.
5. Constructs the configured LLM backend from environment variables.
6. Restores the owner-scoped, bounded SQLite job store for async assessments.
7. Performs an LLM health check and logs whether model-backed steps are available.
...
| `AGENTYZER_OPENWEBUI_TOOL_CALLS` | `auto` | Native OpenAI-style research tool calls for OpenWebUI, or `off` to use text `FETCH_*` and `CLONE_REPOSITORY` directives. |
...
For models with a known window, set `AGENTYZER_OPENWEBUI_CONTEXT_WINDOW` to
enable preflight prompt compaction before the request is sent. The estimate is
...
131072-token context can use `AGENTYZER_OPENWEBUI_CONTEXT_WINDOW=131072`.
