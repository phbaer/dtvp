...
- `dtvp/vulnerability_support_services.py` merges detailed vulnerability data into findings and caches tmrescore results.
...
- `test_setup/` contains mock Dependency-Track, VScorer/tmrescore, and code-analysis services for local and test workflows. `test_setup/mock_tmrescore.py` includes the VScorer session, inventory, result, validator, threat-model editor, and session-scoped wizard context/catalog endpoints used by DTVP integration tests.
...
- `frontend/src/types.ts` defines project, grouped vulnerability, assessment, statistics, cache, VScorer/tmrescore, wizard context/catalog, and proposal shapes.
...
- Main local URLs in the pm2 mock stack: frontend `http://localhost:5173`, backend `http://localhost:8000`, mock Dependency-Track `http://localhost:8081`, mock tmrescore `http://localhost:8090/ui`.
...
- Cache and concurrency: `DTVP_DT_CACHE_PATH`, `DTVP_DT_CACHE_REFRESH_SECONDS`, `DTVP_VERSION_FETCH_CONCURRENCY`, `DTVP_TMRESCORE_CACHE_PATH`, `DTVP_TMRESCORE_TASK_TTL_SECONDS`, `DTVP_ANALYSIS_QUEUE_TTL_SECONDS`.
- VScorer/TMRescore: `DTVP_TMRESCORE_URL`, `DTVP_TMRESCORE_TIMEOUT_SECONDS`, `DTVP_TMRESCORE_OLLAMA_MODEL`.
