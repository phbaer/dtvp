...
The app can run against a real Dependency-Track instance or the bundled mock services in `test_setup/`. Optional integrations add threat-model based rescoring through tmrescore and code-analysis backed vulnerability assessment.
...
- Roles: `ANALYST` submissions get `[Status: Pending Review]`; `REVIEWER` can review, approve, use Settings, and access tmrescore. Role mapping lives in `data/user_roles.json`.
...
- TMRescore is enabled by `DTVP_TMRESCORE_URL`. `frontend/src/pages/TMRescore.vue`, `dtvp/tmrescore_routes.py`, `dtvp/tmrescore_inventory_services.py`, `dtvp/tmrescore_execution_services.py`, and `dtvp/tmrescore_integration.py` build either a latest-only or merged multi-version synthetic CycloneDX SBOM, upload `.tm7` plus optional `items.csv` and config files, poll progress, cache results, and expose per-project proposals to the Project View.
...
- `test_setup/` contains mock Dependency-Track, tmrescore, and code-analysis services for local and test workflows.
...
- TMRescore: `DTVP_TMRESCORE_URL`, `DTVP_TMRESCORE_TIMEOUT_SECONDS`, `DTVP_TMRESCORE_OLLAMA_MODEL`.
