...
class AuthSettings(BaseSettings):
    ...
    # Compatibility properties used by the application middleware. The
    # hardened flow keeps its transaction cookie scoped to the application,
    # while the legacy Authlib middleware remains scoped to the auth routes.
    @property
    def oidc_cookie_path(self) -> str:
        return f"{self.cookie_path.rstrip('/')}/auth" if self.cookie_path != "/" else "/auth"
