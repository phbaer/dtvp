...
class CodeAnalysisStoreSettings:
    ...
    def from_env(
        ...
        return cls(
            ...
            dashboard_cache_seconds=_integer(
                source,
...
class DurableStorageSettings:
    ...
    project_list_ttl_seconds: int = 30
    ...
    def from_env(
        ...
        return cls(
            ...
            project_list_ttl_seconds=_integer(
                source,
                "DTVP_VULNERABILITY_BACKEND_PROJECT_LIST_TTL_SECONDS",
                _integer(
                    source,
                    "DTVP_DT_PROJECT_LIST_TTL_SECONDS",
                    30,
                    minimum=0,
                ),
                minimum=0,
...
class TaskRuntimeSettings:
    grouped_vuln_ttl_seconds: int = 900
    grouped_vuln_max_retained: int = 24
    group_query_workers: int = 4
    group_query_max_pending: int = 8
    group_build_workers: int = 2
    group_build_max_pending: int = 4
    group_postprocess_workers: int = 1
    group_postprocess_max_pending: int = 2
    group_detail_workers: int = 2
    group_detail_max_pending: int = 8
    tmrescore_ttl_seconds: int = 3600
    analysis_queue_ttl_seconds: int = 3600
    analysis_queue_capacity: int = 1
    analysis_queue_max_pending: int = 1000
    ...
    def from_env(cls, environment: Environment | None = None) -> "TaskRuntimeSettings":
        ...
        return cls(
            ...
            grouped_vuln_max_retained=_integer(
                source,
                "DTVP_GROUPED_VULN_TASK_MAX_RETAINED",
                24,
                minimum=1,
            ),
            group_query_workers=_integer(
                source,
                "DTVP_GROUP_QUERY_WORKERS",
                4,
                minimum=1,
            ),
            group_query_max_pending=_integer(
                source,
                "DTVP_GROUP_QUERY_MAX_PENDING",
                8,
                minimum=0,
            ),
            group_build_workers=_integer(
                source,
                "DTVP_GROUP_BUILD_WORKERS",
                2,
                minimum=1,
            ),
            group_build_max_pending=_integer(
                source,
                "DTVP_GROUP_BUILD_MAX_PENDING",
                4,
                minimum=0,
            ),
            group_postprocess_workers=_integer(
                source,
                "DTVP_GROUP_POSTPROCESS_WORKERS",
                1,
                minimum=1,
            ),
            group_postprocess_max_pending=_integer(
                source,
                "DTVP_GROUP_POSTPROCESS_MAX_PENDING",
                2,
                minimum=0,
            ),
            group_detail_workers=_integer(
                source,
                "DTVP_GROUP_DETAIL_WORKERS",
                2,
                minimum=1,
            ),
            group_detail_max_pending=_integer(
                source,
            ...
            analysis_queue_max_pending=_integer(
                source,
