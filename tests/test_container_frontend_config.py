...
def test_runtime_config_is_not_interpolated_by_the_shell():
    ...
    ci_index_html = (ROOT / "tests" / "assets" / "index.html").read_text(
        encoding="utf-8"
    )
    ...
    assert '<script src="/runtime-config.js"></script>' in ci_index_html
    assert "window.__env__ =" not in index_html
    assert "window.__env__ =" not in ci_index_html
