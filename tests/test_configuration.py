...
def test_invalid_numbers_fall_back_and_valid_numbers_respect_floors():
    ...
    tasks = TaskRuntimeSettings.from_env(
        {
            ...
            "DTVP_GROUPED_VULN_TASK_MAX_RETAINED": "0",
            "DTVP_GROUP_QUERY_WORKERS": "0",
            "DTVP_GROUP_QUERY_MAX_PENDING": "-1",
            "DTVP_GROUP_BUILD_WORKERS": "0",
            "DTVP_GROUP_BUILD_MAX_PENDING": "-1",
            "DTVP_GROUP_POSTPROCESS_WORKERS": "0",
            "DTVP_GROUP_POSTPROCESS_MAX_PENDING": "-1",
            "DTVP_GROUP_DETAIL_WORKERS": "0",
            "DTVP_GROUP_DETAIL_MAX_PENDING": "-1",
            "DTVP_TMRESCORE_TASK_TTL_SECONDS": "invalid",
            "DTVP_ANALYSIS_QUEUE_CAPACITY": "0",
            "DTVP_ANALYSIS_QUEUE_MAX_PENDING": "0",
        }
    )
    ...
    assert tasks.grouped_vuln_max_retained == 1
    assert tasks.group_query_workers == 1
    assert tasks.group_query_max_pending == 0
    assert tasks.group_build_workers == 1
    assert tasks.group_build_max_pending == 0
    assert tasks.group_postprocess_workers == 1
    assert tasks.group_postprocess_max_pending == 0
    assert tasks.group_detail_workers == 1
    assert tasks.group_detail_max_pending == 0
    assert tasks.tmrescore_ttl_seconds == 3600
    assert tasks.analysis_queue_capacity == 1
    assert tasks.analysis_queue_max_pending == 1
    ...
    generic = DurableStorageSettings.from_env(
        {
            ...
            "DTVP_VULNERABILITY_BACKEND_PROJECT_LIST_TTL_SECONDS": "90",
