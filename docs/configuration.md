...
| `DTVP_VULNERABILITY_BACKEND_PROJECT_LIST_TTL_SECONDS` | Freshness window for serving the complete cached project list without an upstream request | `30` |
| `DTVP_DT_ACTIVE_PROJECT_TTL_SECONDS` | Legacy-compatible idle window for periodic Dependency-Track project refresh | `900` |
| `DTVP_DT_ACTIVE_PROJECT_LIMIT` | Legacy-compatible limit for recent Dependency-Track projects eligible for periodic refresh | `8` |
| `DTVP_DT_MEMORY_CACHE_MAX_ENTRIES` | Legacy-compatible process-local LRU file-object cache limit | `256` |
| `DTVP_DT_PROJECT_QUERY_CACHE_MAX_ENTRIES` | Legacy-compatible process-local named-project query LRU limit | `128` |
| `DTVP_ASSESSMENT_OUTBOX_PATH` | Transactional assessment overlay and synchronization outbox | `<DTVP_VULNERABILITY_BACKEND_CACHE_PATH>/assessment_outbox.sqlite` |
| `DTVP_ASSESSMENT_SYNC_CONCURRENCY` | Global concurrent background assessment writes to the selected backend | `4` |
| `DTVP_ASSESSMENT_STRICT_DT_CONFLICTS` | Read live assessment state before accepting a local save | `false` |
