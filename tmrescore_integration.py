...
def normalize_tmrescore_proposal(proposal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    normalized["rescored_vector_adjusted"] = False

    # Sanitize: ensure rescored vector preserves original base metrics
    if normalized["original_vector"] and normalized["rescored_vector"]:
        corrected = sanitize_rescored_vector(
            normalized["original_vector"], normalized["rescored_vector"]
        )
        if corrected != normalized["rescored_vector"]:
            normalized["rescored_vector_adjusted"] = True
        normalized["rescored_vector"] = corrected
