...
| Frontend UI tests | `cd frontend && npm run test:ui` |
...
The Playwright container version in `.github/workflows/build-publish.yml` must
match the `@playwright/test` version resolved in `frontend/package-lock.json` so
the CI image provides the browser revision expected by the test runner.
