...
describe('env.ts', () => {
    ...
    it('returns default value if window.__env__ is undefined', () => {
        expect(getRuntimeConfig('DTVP_API_URL', 'default')).toBe('default')
    })
    ...
    it('returns valid value if present', () => {
        (window as any).__env__ = {
            DTVP_API_URL: 'https://api.example.com'
        }
        expect(getRuntimeConfig('DTVP_API_URL', 'default')).toBe('https://api.example.com')
    })
})
