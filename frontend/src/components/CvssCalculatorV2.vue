...
const structuredGroups = computed(() => {
  ...
  try {
    ...
    const mkRow = (base: string, req?: string) => ({
      base: getComp(base),
...
<template>
  <div v-if="structuredGroups" class="space-y-6">
    <!-- Base & Requirement Metrics Table -->
        ...
        <table class="w-full text-left border-collapse text-xs">
          <thead>
            <tr class="text-gray-500 uppercase font-bold bg-gray-800/30 border-b border-gray-700/50">
              <th class="py-2 px-3 w-1/3">Metric</th>
              <th class="py-2 px-3 w-1/3">Base (Read Only)</th>
              <th class="py-2 px-3 w-1/3 text-indigo-300">Requirement</th>
            </tr>
          </thead>
          <tbody>
